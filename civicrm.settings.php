...
if (PHP_SAPI == 'cli' || !class_exists('Platformsh\ConfigReader\Config')) {
...
if ($platformsh->inRuntime()) {
    ...
    if ($platformsh->hasRelationship('civicrm')) {
        $credCivi = $platformsh->credentials('civicrm');
        $civicrm_database_user = $credCivi['username'];
        $civicrm_database_password = $credCivi['password'];
        $civicrm_database_name = $credCivi['path'];
        $civicrm_database_host = $credCivi['host'];
        $civicrm_database_port = $credCivi['port'];
    }
}
// Not in platform runtime. Set local variables.
    ...
    if (file_exists(__DIR__.'/../../../../.env')) {
        ...
        (new Symfony\Component\Dotenv\Dotenv())->usePutenv(true)->bootEnv(__DIR__.'/../../../../.env');
    }
    $repo_root                 = getenv('REPO_ROOT');
    $site_uri                  = getenv('SITE_URI') . '/';
    $cms_database_user         = getenv('DB_USER');
    $cms_database_password     = getenv('DB_PASSWORD');
    $cms_database_name         = getenv('DB_NAME');
    $cms_database_host         = getenv('DB_HOST');
    $civicrm_database_user     = getenv('DB_USER');
    $civicrm_database_password = getenv('DB_PASSWORD');
    $civicrm_database_name     = getenv('CIVI_DB_NAME');
    $civicrm_database_host     = getenv('DB_HOST');
}
...
if (!defined('CIVICRM_UF_DSN') && CIVICRM_UF !== 'UnitTests') {
    define( 'CIVICRM_UF_DSN', 'mysql://' . $cms_database_user . ':' . $cms_database_password  . '@' . $cms_database_host . ':3306/' . $cms_database_name  . '?new_link=true');
}

// Additional settings generated by installer:
$civicrm_paths['civicrm.private']['path'] = $repo_root . '/web/app/uploads/civicrm';
...
if (!defined('CIVICRM_TEMPLATE_COMPILEDIR')) {
  define('CIVICRM_TEMPLATE_COMPILEDIR', $repo_root . '/web/app/uploads/civicrm/templates_c');
}
...
if (!defined('CIVICRM_UF_BASEURL')) {
  define('CIVICRM_UF_BASEURL', $site_uri . 'wp');
}
...
if (!defined('CIVICRM_CRED_KEYS')) {
  define('_CIVICRM_CRED_KEYS', getenv('CIVICRM_CRED_KEYS'));
...
if (!defined('CIVICRM_DEPLOY_ID')) {
  define('_CIVICRM_DEPLOY_ID', getenv('CIVICRM_DEPLOY_ID'));
...
if (!defined('CIVICRM_SIGN_KEYS')) {
  define('_CIVICRM_SIGN_KEYS', getenv('CIVICRM_SIGN_KEYS'));
...
// define('CIVICRM_MAIL_LOG', $repo_root . '/web/app/uploads/civicrm/templates_c/mail.log');
// }
